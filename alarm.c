...
alarm_task (void * p_arg)
{
    ...
    for (;;)	
    {
        ...
        alarmEvents = OSFlagPend(&g_alarm_flags, 0xF, 0, OS_OPT_PEND_FLAG_SET_ANY + OS_OPT_PEND_FLAG_CONSUME + OS_OPT_PEND_BLOCKING, NULL, &err);
        assert(OS_ERR_NONE == err);
        ...
        else if (ALARM_MEDIUM == alarmEvents)
        {
        ...
        else if (ALARM_NONE == alarmEvents)
        {
